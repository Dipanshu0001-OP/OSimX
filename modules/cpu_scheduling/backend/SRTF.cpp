...
SchedulingResult SRTF::schedule(std::vector<Process> processes)
{
    ...
    while (completed < n) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (selected == -1 ||
                p.remainingTime < result.processes[selected].remainingTime ||
                (p.remainingTime == result.processes[selected].remainingTime && p.id < result.processes[selected].id)) {
                selected = i;
            }
        }
...
std::string SRTF::getAlgorithmName() const
{
